...
def make_features_table(year, quarter):
    ...
    last_4_quarters = ookla.speed_data(ookla.available_files().loc[(connection_type,year,quarter):(connection_type,next_year,next_quarter)].path)
    ...
    features_table = features_table[features_table['tile_frac'] > 0.35] #may need to delete
    
    #Add population column
    features_table['population_in_tile'] = (features_table['tile_area'] * features_table['POP_DENSITY'] / features_table['tile_frac'])/1000000
...
if __name__ == "__main__":
    ...
    total_people = sum(features_table['POP_DENSITY'] * features_table['tile_area'] / features_table['tile_frac'])/1000000
    ...
    for year in years:
        for quarter in quarters:
            ...
            internet_speed_type = 'avg_u_kbps' #'avg_d_kbps'
            
            internet_speed = 10 #50

            features_table = make_features_table(year, quarter)

            #'POP_DENSITY' > 50
            ts_greater_than_50.append(features_table[features_table[internet_speed_type] > internet_speed*1000]['population_in_tile'].sum()/total_people)
            #'POP_DENSITY' < 50
            ts_less_than_50.append(features_table[features_table[internet_speed_type] < internet_speed*1000]['population_in_tile'].sum()/total_people)
            # #'POP_DENSITY' > 50
